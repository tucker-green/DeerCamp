...
const Dashboard = () => {
    ...
    return (
        <div className="space-y-4 sm:space-y-8 pt-2 sm:pt-6 pb-12 sm:pb-20">
            {/* Hero Header */}
            <motion.header
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.6 }}
                className="flex flex-col md:flex-row justify-between items-center md:items-end gap-3 sm:gap-6 mb-4 sm:mb-12 text-center md:text-left"
            >
                <div className="space-y-1.5 sm:space-y-3">
                    ...
                    >
                        <h1 className="text-2xl sm:text-4xl md:text-6xl font-heading font-bold">
                            <span className="text-gray-500 font-light text-xl sm:text-3xl md:text-4xl block mb-1 sm:mb-2">{getGreeting()},</span>
                    ...
                    <motion.p
                        ...
                        className="text-gray-400 text-xs sm:text-lg flex items-center gap-2 justify-center md:justify-start"
                    >
                ...
                <motion.div
                    ...
                    className="glass-panel-strong p-3 sm:p-6 rounded-2xl border-white/10 bg-gradient-to-br from-white/5 to-transparent relative overflow-hidden group hover-lift w-full md:w-auto"
                >
                    ...
                    <div className="relative flex items-center gap-3 sm:gap-6 justify-center md:justify-start">
                        {weather ? (
                            <>
                                <div className="text-center">
                                    <div className="flex items-start justify-center gap-1">
                                        <Thermometer size={20} className="text-green-400 mt-1" />
                                        <div className="text-3xl sm:text-5xl font-bold font-heading bg-gradient-to-br from-white to-gray-400 bg-clip-text text-transparent">{weather.temp}°</div>
                                    </div>
                                ...
                                <div className="h-10 sm:h-14 w-px bg-gradient-to-b from-transparent via-white/20 to-transparent" />
                            ...
                            <div className="flex items-center justify-center w-full h-full py-2">
                                {weatherLoading ? (
                                    ...
                                    <div className="text-gray-400 text-xs sm:text-sm flex flex-col items-center">
                                        <span>No location set</span>
                                        <span className="text-[10px] text-gray-500">Add club location</span>
                                    </div>
                                )}
                            </div>
                        )}
            ...
            <motion.div
                ...
                className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-8 mt-4 sm:mt-8"
            >
                {/* Main Content Area */}
                    ...
                    <section>
                        <div className="flex justify-between items-center mb-4 sm:mb-6">
                            <div>
                                ...
                                <p className="text-xs sm:text-sm text-gray-500 mt-1">Recent club updates</p>
                            </div>
                            <motion.button
                                whileHover={{ scale: 1.05, x: 3 }}
                                whileTap={{ scale: 0.95 }}
                                onClick={() => navigate('/feed')}
                                className="text-xs sm:text-sm text-green-400 hover:text-green-300 font-semibold transition-colors flex items-center gap-1 px-2.5 sm:px-3 py-1.5 rounded-lg hover:bg-green-500/10"
                            >
                                View All
                        ...
                        <div className="space-y-3">
                            {recentPosts.length === 0 ? (
                                <div className="glass-panel-strong p-6 sm:p-8 rounded-2xl border border-white/10 text-center">
                                ...
                                recentPosts.map((post, i) => (
                                    <motion.div
                                        ...
                                        whileHover={{ x: 4 }}
                                        onClick={() => navigate('/feed')}
                                        className="glass-panel-strong p-4 sm:p-5 rounded-2xl border border-white/10 hover:border-white/20 transition-all duration-300 flex items-center justify-between group relative overflow-hidden cursor-pointer"
                                    >
                                        ...
                                        <div className="flex items-center gap-4 relative z-10 flex-1 min-w-0">
                                            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-gradient-to-br from-green-600 to-green-800 flex items-center justify-center text-sm font-bold shadow-lg ring-2 ring-white/10 group-hover:ring-green-500/30 transition-all flex-shrink-0">
                ...
                <div className="space-y-6">
                    <motion.section
                        ...
                        className="glass-panel-strong p-5 sm:p-6 rounded-2xl border border-white/10 relative overflow-hidden group"
                    >
                        ...
                        <div className="relative z-10">
                            <h3 className="text-lg sm:text-xl font-heading font-bold mb-5 flex items-center gap-2">
                    ...
                    >
                        <h3 className="text-lg sm:text-xl font-heading font-bold mb-5">Quick Actions</h3>
...
const StatCard = ({ icon, label, value, trend, color, bg, onClick }: { icon: React.ReactNode, label: string, value: string, trend: string, color: string, bg: string, onClick?: () => void }) => (
    <motion.div
        ...
        onClick={onClick}
        className="glass-panel-strong p-3 sm:p-6 rounded-2xl border border-white/10 relative overflow-hidden group cursor-pointer"
    >
        ...
        <div className="relative z-10">
            <div className={`w-9 h-9 sm:w-12 sm:h-12 rounded-2xl ${bg} ${color} flex items-center justify-center mb-3 sm:mb-4 border border-white/10 shadow-lg group-hover:scale-110 group-hover:rotate-3 transition-all duration-300`}>
                {icon}
            </div>
            <p className="text-gray-500 text-[10px] sm:text-xs font-semibold uppercase tracking-wider mb-0.5">{label}</p>
            <div className="flex items-baseline gap-2 mt-1.5">
                <h4 className="text-2xl sm:text-4xl font-heading font-bold bg-gradient-to-br from-white to-gray-400 bg-clip-text text-transparent">{value}</h4>
            </div>
            <div className="mt-1.5 sm:mt-3 flex items-center gap-1.5 text-[10px] sm:text-xs font-medium">
                <div className="flex items-center gap-1 px-2 py-0.5 sm:py-1 rounded-full bg-green-500/10 text-green-400 border border-green-500/20">
...
const ActionButton = ({ label, onClick }: { label: string, onClick?: () => void }) => (
    <motion.button
        ...
        whileTap={{ scale: 0.95 }}
        onClick={onClick}
        className="p-3 sm:p-4 bg-gradient-to-br from-white/10 to-white/5 hover:from-white/15 hover:to-white/10 border border-white/10 hover:border-white/20 rounded-xl transition-all text-xs sm:text-sm font-semibold text-gray-300 hover:text-white shadow-lg hover:shadow-xl relative overflow-hidden group"
    >
